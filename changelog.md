...
- Upgrades `@mdx-js/loader`, `@mdx-js/tag`, `@types/node`, `@types/react`, `css-loader`, `husky`, `next`, `next-offline`, `next-server`, `react`, `react-dom`, `sass`, and `webpack` to latest versions
